...
export default async function AccountPage({ searchParams }: { searchParams: Promise<{ error?: string; saved?: string; request?: string; payment?: string }> }) {
  ...
  const [params, profileResult, adminResult, instancesResult, reviewsResult, requestsResult] = await Promise.all([
    ...
    client.from('service_requests').select('id, service_title, payment_method, payment_status, status, timezone, scheduled_at').eq('customer_id', user.id).order('created_at', { ascending: false }),
  ]);
  const profile = profileResult.data;
  const errors: Record<string, string> = { name: 'Choose a name between 2 and 60 characters.', save: 'Your name could not be saved.', review: 'Please write 20–1,600 characters.', request: 'Please complete the payment and scheduling details.' };
  ...
  return <section className="account-page"><div className="account-panel">
    ...
    <div className="account-next"><h2>Your appointments</h2>{requestsResult.error ? <p>Appointments are being prepared.</p> : requestsResult.data?.length ? <div className="eligible-services">{requestsResult.data.map(request => <div key={request.id}><span><strong>{request.service_title}</strong><small>{request.scheduled_at ? appointmentTime(request.scheduled_at, request.timezone) : request.payment_method}</small><small>Payment: {request.payment_status.replace('_', ' ')}</small></span><span>{request.status === 'scheduled' && request.payment_status === 'awaiting_verification' ? 'Reserved' : request.status}</span></div>)}</div> : <p>No appointments yet.</p>}</div>
