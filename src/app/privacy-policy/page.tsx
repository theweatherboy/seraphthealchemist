...
const sections: LegalSection[] = [
  ...
  { id: 'saved-birth-details', title: 'Optional saved birth details', content: <>
    <p>Signed-in users can choose an unchecked permission box and select Save to retain one birth profile in their private account: birth date, local birth time, selected city label, coordinates, and its resolved timezone. We also record the permission version and time of the save. Calculating a chart or sending an AI question does not save these fields automatically.</p>
    <p>Saved birth profiles are not public and are not included in public testimonies or the website administration interface. They can be loaded into the birth-chart form for a fresh calculation, replaced through another explicit save, or deleted on that page. Deleting the account also deletes its saved profile. Deleting saved details removes the active database record and clears the current page’s chart and chat; infrastructure backups may retain earlier copies under the provider’s retention settings.</p>
    <p>The AI assistant receives selected calculated placements and transits when you send a question. Saving birth details does not save chat conversations or send a question automatically, and deleting them does not remove any prior AI-provider logs.</p>
  </> },
  ...
  { id: 'providers', title: 'Service providers & disclosures', content: <>
    ...
    <ul>
      ...
      <li><strong>CosmyDay:</strong> when you search for a birthplace, the city text you enter is sent to CosmyDay to find matching places and coordinates. When you request a reading, your birth date, local birth time, and selected birthplace coordinates are sent to its API to calculate the natal chart. Its service processes this information under its own privacy policy. We save your birth details in our database only if you separately choose the optional account-saving feature.</li>
      <li><strong>Astronomy Engine:</strong> the site uses this open-source calculation library on our server to calculate future planetary positions and transit-to-natal aspect dates. Birth details are not sent to Astronomy Engine as a separate service.</li>
      <li><strong>Groq:</strong> if you send a question to the optional birth-chart AI assistant, your question, recent conversation, calculated chart placements, timezone, and selected transit events are sent to Groq to generate a reply. We do not automatically include your birth date, birth time, or birthplace text in that request. Anything you type into chat is included. We keep the conversation in the current page rather than our database. Groq may retain data under its reliability and abuse monitoring policies; see its <a href="https://console.groq.com/docs/your-data" target="_blank" rel="noreferrer">data controls</a>. Clearing chat removes it from the page, not from any provider logs.</li>
    ...
    <p>Birth chart readings are optional. Do not submit details you do not want processed by CosmyDay. Read the <a href="https://cosmyday.com/privacy-policy" target="_blank" rel="noreferrer">CosmyDay privacy policy</a> and the <a href="https://cosmyday.com/api-docs" target="_blank" rel="noreferrer">CosmyDay API documentation</a>.</p>
